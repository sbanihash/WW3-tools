...
def orgensemblesat(*args):
	...
	The format must be WW3.Altimeter_YYYYMM_c00 for the control member
	and WW3.Altimeter_YYYYMM_p01 for all the perturbed members, saved
	...
	Example:
	 ...
	 fdatacontrol,fhs,fwnd=wproc.orgensemblesat('list.txt',11)
	  ...
	  fdatacontrol['mhs'].values is the array with model results of Hs 
	...
	for i in range(0,np.size(flist)):
		...
		for j in np.append(ensm,ensm[0]):
			if '/' in flist[i]:
				if j==0:
					os.system("ln -fs "+"/".join(flist[i].split('/')[0:-1])+"/"+"_".join(flist[i].split('/')[-1].split('_')[0:2])+"_mem"+str(j).zfill(3)+"_*.nc"+" auxorgensemblesat.nc")
				else:
					os.system("ln -fs "+"/".join(flist[i].split('/')[0:-1])+"/"+"_".join(flist[i].split('/')[-1].split('_')[0:2])+"_mem"+str(j).zfill(3)+"_*.nc"+" auxorgensemblesat.nc")
			else:
				if j==0:
		...
		for j in ensm:
			...
			if '/' in flist[i]:
				...
				if j==0:
					os.system("ln -fs "+"_".join(flist[i].split('_')[0:2])+"_mem"+str(j).zfill(3)+"_*.nc"+" auxorgensemblesat.nc")
				else:
					os.system("ln -fs "+"_".join(flist[i].split('_')[0:2])+"_mem"+str(j).zfill(3)+"_*.nc"+" auxorgensemblesat.nc")
