...
def qqplot(*args):
	...
	ax.plot(gaussian_filter(qobs[0,:],1),gaussian_filter(qm[0,:],1), color=ccol[0], marker=mmark[0], linestyle='--',label=mlabels[0],linewidth=1.,alpha=0.8,zorder=3)
	ax.fill_between(gaussian_filter(qobs[0,:],1),gaussian_filter(qm[1,:],1),gaussian_filter(qm[2,:],1), color='lightblue', label=mlabels[1],alpha=0.5,zorder=3)
...
def scatterplot(*args):
	...
	if obs[0,:].shape[0]>50000:
		sk=int(np.round(float(obs[0,:].shape[0])/30000.,0))
	else:
		sk=1
	...
	if len(mlabels) > 0 and mlabels[0] != '': #mlabels[0] != '':
		plt.legend(loc="upper left",fontsize=sl-2)
